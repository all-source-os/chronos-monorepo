...
| 4 | Events page | Live feed | WebSocket connection may show "simulation mode" when Core WS endpoint is unreachable. Tests handle both real and simulated modes gracefully. | Known/Acceptable | N/A — by design for demo accounts without live WS |
...
| 7 | Replay page | Start Replay | Replay requires valid projection targets. Demo accounts may not have projections — test handles empty state. | Known/Acceptable | N/A — correct behavior for new accounts |
...
| 10 | Audit Log | Filter pills | Audit log entries are generated on tenant-scoped actions. New demo accounts start with empty audit logs — test handles empty state. | Known/Acceptable | N/A |

## Summary

- **4 issues fixed** — all stemming from the tenant sync root cause (QS couldn't find demo tenants in Core after restart)
- **3 known/acceptable** — expected behavior for new demo accounts (empty states, simulation mode)
