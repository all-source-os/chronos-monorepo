...
**Admin app full env checklist (Vercel `allsource-admin`, production):** `NEXT_PUBLIC_API_URL=https://api.all-source.xyz` (server-side fallbacks), `NEXT_PUBLIC_APP_URL=https://admin.all-source.xyz` (OAuth `redirect_to`), `CONTROL_PLANE_INTERNAL_URL=https://api.all-source.xyz` (BFF target). **Control Plane (`allsource-control-plane` Fly):** `ALLOWED_FRONTEND_URLS` includes `https://admin.all-source.xyz`, `OAUTH_COOKIE_DOMAIN=.all-source.xyz`, and the operator's email in `ADMIN_EMAILS`.
