...
3. Event-sourced memory for agents. ~12μs recall. MIT.
...
> AllSource is durable memory for AI agents. It records every event your agent emits to a write-ahead log + Parquet (survives restarts), then serves recall from in-memory projections in ~12μs — fast enough to hit every turn. 469K events/sec, 43 MCP tools for Claude, self-host free (MIT).
...
469K events/sec, 43 MCP tools so Claude reads and writes memory directly, and x402 so the agent pays per call instead of you renting capacity you don't use.

It's MIT-licensed — self-host the whole thing free, forever, on your own hardware. Hosted starts at $19/mo if you'd rather I run it. There are honest comparison pages up too (vs mem0, Zep, Letta) if you're already using one.
...
- 🔌 **43 MCP tools** — drop into Claude Desktop; the agent reads and writes memory directly.
- 🚀 **469K events/sec** — the memory layer is never the bottleneck.
- 🆓 **Self-host free (MIT)** — unlimited events on your hardware. Hosted from $19/mo.
...
---
**Closer for any linked post:** `Self-host free. $19/mo hosted. MIT. → all-source.xyz`
