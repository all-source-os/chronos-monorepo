# Chronon — Event-Sourced Task CLI for Autonomous Agents
...
**Chronon** — the smallest indivisible quantum of time. In physics, a chronon is the hypothetical atom of duration — time cannot be subdivided further. In the AllSource universe, every event is a chronon: an atomic, immutable moment in the timeline from which all state is derived.
...
Chronon is a single Rust binary (~30MB) that wraps Embedded Core. It stores events in `.chronon/` at the project root — no Docker, no server, no configuration required.
...
```bash
cn init                                  # Create .chronon/ with embedded Core
...
# Migration
cn migrate-beads                         # Import .beads/ → .chronon/ events
```
...
**Key insight**: No new event types needed. Chronon reuses the schemas already defined in Embedded Core phases 5–7.
...
```toml
# apps/chronon/Cargo.toml (new crate in apps/)
[package]
name = "chronon"
version = "0.1.0"
...
**Build**: `cargo build --release -p chronon` → `~30MB` binary (no server deps).

**Distribution**:
- GitHub Releases (prebuilt binaries for linux-amd64, linux-arm64, darwin-arm64, darwin-amd64)
- `cargo install chronon` (from crates.io, after Rust SDK publishing rules)
- Homebrew tap (future)
...
|----------|--------|------------------------|-----------|
| Name | Chronon (`cn`) | AllSpark, Forge, Arc, Nexus, Axiom, Vectis | Chronon = smallest quantum of time; echoes Chronos monorepo + Vector Prime lineage; `cn` is short and unambiguous |
| Storage dir | `.chronon/` | `.allsource/`, `.cn/` | Distinct from `.allsource/` (cloud config); `.cn/` too terse; `.chronon/` self-documenting |
...
| Crate location | `apps/chronon/` | `sdks/`, `tooling/` | It's a deployable binary, belongs in `apps/` per monorepo rules |
...
Beyond replacing beads-rust, Chronon becomes the **local event store for any CLI workflow**:
...
- **MCP tool auditing**: MCP servers emit tool events → Chronon aggregates success rates locally
- **Multi-repo orchestration**: `cn sync` between repos → unified view of work across projects

Every event is a chronon. Every chronon is immutable. The timeline remembers everything.
