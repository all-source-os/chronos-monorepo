...
async fn create_and_list_task() {
    ...
    repo.create_task("t-0001", "Write tests", "p1", &[], TaskType::Task, None, None)
        .await
        .unwrap();
...
async fn full_lifecycle_create_claim_done() {
    let repo = setup().await;
    repo.create_task("t-0001", "Build feature", "p0", &[], TaskType::Feature, None, None)
        .await
        .unwrap();
...
async fn ready_excludes_blocked_tasks() {
    ...
    repo.create_task("t-0001", "Dep", "p2", &[], TaskType::Task, None, None)
        .await
        .unwrap();
    repo.create_task("t-0002", "Blocked", "p2", &["t-0001".to_string()], TaskType::Task, None, None)
        .await
        .unwrap();
...
async fn epic_with_children() {
    let repo = setup().await;
    repo.create_task("t-epic", "Auth System", "p0", &[], TaskType::Epic, None, None)
        .await
        .unwrap();
    repo.create_task("t-child1", "Login flow", "p1", &[], TaskType::Task, Some("t-epic"), None)
        .await
        .unwrap();
    repo.create_task("t-child2", "Signup flow", "p2", &[], TaskType::Task, Some("t-epic"), None)
        .await
        .unwrap();
    // Non-child task
    repo.create_task("t-other", "Unrelated", "p3", &[], TaskType::Task, None, None)
        .await
        .unwrap();
