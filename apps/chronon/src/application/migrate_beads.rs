...
fn beads_id_to_chronon(beads_id: &str) -> String {
...
fn beads_priority_to_chronon(p: u8) -> &'static str {
    match p {
        0 | 1 => "p0",
        2 => "p1",
        3 => "p2",
...
pub async fn migrate_beads(
    ...
    for issue in &issues {
        let id = beads_id_to_chronon(&issue.id);
        let priority = beads_priority_to_chronon(issue.priority);
        ...
        let parent = issue
            ...
            .map(|d| beads_id_to_chronon(&d.depends_on_id));
    ...
    for issue in &issues {
        ...
        for dep in &issue.dependencies {
            if dep.dep_type == "blocks" {
                let blocker_id = beads_id_to_chronon(&dep.depends_on_id);
