...
impl EventStreamRepository for PostgresEventStreamRepository {
    ...
    async fn get_watermark(&self, stream_id: &EntityId) -> Result<u64> {
        let watermark: i64 =
            sqlx::query_scalar("SELECT watermark FROM event_streams WHERE stream_id = $1")
                .bind(stream_id.as_str())
                .fetch_one(&self.pool)
                .await
    ...
    async fn verify_gapless(&self, stream_id: &EntityId) -> Result<bool> {
        let is_gapless: bool = sqlx::query_scalar("SELECT verify_stream_gapless($1)")
            .bind(stream_id.as_str())
            .fetch_one(&self.pool)
            .await
    ...
    async fn count_streams_by_tenant(&self, tenant_id: &TenantId) -> Result<usize> {
        let count: i64 =
            sqlx::query_scalar("SELECT COUNT(DISTINCT stream_id) FROM events WHERE tenant_id = $1")
                .bind(tenant_id.as_str())
                .fetch_one(&self.pool)
                .await?;
