...
export const metadata = constructMetadata({
  ...
  description:
    "Use AllSource Prime as an embedded Rust library for knowledge graphs, vector search, and hybrid recall.",
  ...
  return (
    <div className="mx-auto w-full max-w-screen-md px-4 lg:px-8 py-24">
      <h1 className="text-3xl font-bold text-foreground sm:text-4xl mb-2">
        Embed Prime in a Rust application
      </h1>
      <p className="text-lg text-muted-foreground mb-10">
        Use Prime directly as a Rust crate for maximum performance. No network overhead, no server
        process — just a library call.
      </p>

      <div className="prose prose-neutral dark:prose-invert max-w-none space-y-10">
        <section>
          <h2 className="text-xl font-semibold text-foreground mb-3">Add the Dependency</h2>
...
allsource-core = { version = "0.16", features = ["prime"] }`}
        ...
        <section>
          <h2 className="text-xl font-semibold text-foreground mb-3">Feature Flags</h2>
          <p className="text-muted-foreground mb-3">Enable only what you need:</p>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr className="border-b border-muted">
                  ...
                  <td className="p-2">Knowledge graph (nodes, edges, domains, compressed index)</td>
                </tr>
                <tr className="border-b border-muted">
                  <td className="p-2">
                    <code>prime-vectors</code>
                  </td>
                  <td className="p-2">Vector storage and HNSW similarity search</td>
                </tr>
                <tr className="border-b border-muted">
                  <td className="p-2">
                    <code>prime-recall</code>
                  </td>
                  <td className="p-2">Hybrid recall engine (vector + graph + temporal)</td>
                </tr>
                <tr className="border-b border-muted">
                  <td className="p-2">
                    <code>prime-full</code>
                  </td>
                  <td className="p-2">All of the above combined</td>
                </tr>
              </tbody>
            </table>
          </div>
...
allsource-core = { version = "0.16", features = ["prime-full"] }`}
        ...
        <section>
          <h2 className="text-xl font-semibold text-foreground mb-3">Knowledge Graph Basics</h2>
...
fn main() -> anyhow::Result<()> {
    ...
    Ok(())
        ...
        <section>
          <h2 className="text-xl font-semibold text-foreground mb-3">Vector Search</h2>
          <p className="text-muted-foreground mb-3">
...
for result in &results {
    println!("score={:.3} text={}", result.score, result.text);
        ...
        <section>
          ...
          <p className="text-muted-foreground mb-3">
            Combine vector similarity, graph expansion, and temporal recency into a single query.
            Requires the <code>prime-recall</code> feature:
          </p>
          <pre className="rounded-lg bg-black/80 p-4 text-xs text-green-400 overflow-x-auto">
...
println!("domains: {:?}, nodes: {}", summary.domains, summary.total_nodes);`}
        ...
        <section>
          <h2 className="text-xl font-semibold text-foreground mb-3">Persistence</h2>
          <p className="text-muted-foreground leading-relaxed">
            <code>Prime::open(&quot;path&quot;)</code> opens a durable store backed by WAL
            (Write-Ahead Log) and Parquet. Data survives crashes and restarts:
          </p>
          <ul className="space-y-1 text-sm text-muted-foreground mt-3">
            <li>
              <strong>WAL:</strong> CRC32 checksums, configurable fsync (default 100ms), automatic
              crash recovery
            </li>
            <li>
              <strong>Parquet:</strong> Periodic flush with Snappy compression for long-term
              columnar storage
            </li>
            <li>
              <strong>DashMap:</strong> In-memory concurrent map rebuilt from WAL on startup for
              fast reads
            </li>
          </ul>
          <p className="text-muted-foreground leading-relaxed mt-3">
            Call <code>prime.shutdown()</code> for graceful shutdown. On crash, WAL replay recovers
            all committed events automatically.
          </p>
        </section>

        <section>
          <h2 className="text-xl font-semibold text-foreground mb-3">When to Use Each Mode</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
          ...
          <p className="text-sm text-muted-foreground mt-3">
            Use embedded when Prime is a core part of your Rust application. Use MCP for AI agent
            integrations. Use HTTP when you need language-agnostic access or want to share a single
            Prime instance across multiple clients.
          </p>
        </section>
      </div>
    </div>
