...
Local-only memory remembers across sessions on *this machine*, no account — it's free to run yourself. Cross-machine and team memory are the upgrade — add `--sync-to` + `--api-key` to ship memory to a hosted AllSource Core. A pull, not a gate.
