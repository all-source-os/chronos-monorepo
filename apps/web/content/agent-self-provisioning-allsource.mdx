...
summary: Your agent doesn't need a human to set up storage. One POST to AllSource and it has a tenant, an API key, and 100K events waiting to be written.
...
For teams with multiple agents sharing one tenant, skip self-provisioning and use the [agent key flow](/docs/agent-auth) instead — an admin creates a named key per agent from Settings → Team → Agent Keys.
