...
- **Option A — Claude Desktop Extension** _(ships with v0.21.5)_: download the `.dxt` bundle, double-click it. Claude Desktop installs it natively and prompts for your API key — paste the one from the page into the dialog and you're done. No terminal, no config editing. macOS (Apple Silicon) and Linux x86_64. The .dxt attaches to GitHub Releases automatically from v0.21.5 onward; use Option B until then.

- **Option B — one-line installer** _(recommended today)_: copy the `curl` command the page gives you. It downloads `allsource-prime`, writes the Claude Desktop config (preserving any existing MCP servers you've wired up), and prints the next step.
