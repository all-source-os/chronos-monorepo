...
## The setup (one time, ~30 seconds)

Install the binary and point it at your tenant on the hosted Core so the panel can see what Claude is writing:
...
That's the whole setup. No schemas to define, no migrations, no content types to register up front.
