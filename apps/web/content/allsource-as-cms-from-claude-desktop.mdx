...
Recommended path on macOS Apple Silicon and Linux x86_64. The load-bearing human actions in this path are two: drag the `.dxt` into Claude Desktop, and paste the API key into the install dialog. Two further human-owned steps exist for structural reasons — mint an API key in the browser (Step 3) because the `/connect` endpoint requires a signed-in session, and restart Claude Desktop (Step 5) because the app does not hot-load MCP servers — each filed as a gap with a follow-up bead in `docs/proposals/AGENT_DRIVEN_PRIME_ONBOARDING.md`. Every other step is the agent's job.
...
**Action (agent).** Tell the human:

> "Open [https://www.all-source.xyz/connect](https://www.all-source.xyz/connect) in a browser. If you are not signed in, the page will offer to sign you up (free tier is enough). Once you are signed in, click **Create connection**. The page will display an API key starting with `ask_`. Copy it. Do not close the tab yet — the key is shown only once."

**Action (human).** Visit the URL, sign in or sign up, click the button, copy the key.

**Verify.** Ask the human to confirm they have an API key starting with `ask_` on their clipboard. Do not ask them to paste the key into the conversation — the next step needs it in a different place.

**On failure.** If the human says the page is asking them to sign in and they do not have an account, that is the expected free-tier signup flow — direct them to the **Create free account** button. If the page errors out after they click **Create connection**, ask them to send you the error text and check that the page is up by trying it yourself; the most likely failure is a transient deploy or a network block on their side.

This step is the first of the two unavoidable human actions in this path. See the gap proposal at `docs/proposals/AGENT_DRIVEN_PRIME_ONBOARDING.md` for what would have to ship to remove it.

### Step 4 — Install the `.dxt` extension

**Precondition.** Step 3 left the human with an `ask_...` key on the clipboard.

**Action (agent).** Tell the human:

> "Download [allsource-prime.dxt](https://github.com/all-source-os/all-source/releases/latest/download/allsource-prime.dxt). It is about 37 MB. Once it has downloaded, drag the file into the Claude Desktop window — or open Claude Desktop's Settings, go to **Extensions**, and import the `.dxt` from there. Claude Desktop will open an install dialog. In the **AllSource API Key** field, paste the key from Step 3. Leave the other fields (data directory, sync URL) on their defaults. Click **Install**."
...
This step is the second of the two unavoidable human actions. The drag-and-drop and the API key paste are both physical-input actions the agent cannot perform from inside the conversation.
...
**On failure.** If `prime_stats` is present but errors out, the most likely cause is a bad API key or a sync-to URL the agent's Prime instance cannot reach. Ask the human to reopen `/connect`, mint a fresh key (the old one was shown only once — they probably no longer have it), and re-run Step 4 to overwrite the install with the new key.
...
After merging the snippet, the human still needs to mint an API key (Step 3 above) and restart Claude Desktop (Step 5). Verification (Step 6) is identical.
...
- Connect page (mints the API key): [www.all-source.xyz/connect](https://www.all-source.xyz/connect)
