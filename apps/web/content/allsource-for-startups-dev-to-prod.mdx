...
summary: "The fastest path from zero to event sourcing for startups and side projects. Self-service onboard, a 14-day trial, no infrastructure to manage. Move to production when you're ready — not before."
author: "all.source team"
image: "/assets/blog/allsource-for-startups-dev-to-prod.webp"
category: "use-cases"
---

**You don't need infrastructure to start using AllSource.** One API call gives you a tenant, an API key, and a 14-day trial — no charge until it ends. No Docker, no Kubernetes, no config files. Here's how startups and projects under development should use AllSource — from first prototype to production.
...
**What you have now:** a hosted event store with WAL durability, time-travel queries, and a 14-day trial. No servers to manage. No database to provision. Just HTTP calls.
...
### Pick a paid plan before the trial ends

Your 14-day trial ends once you've validated the idea — pick a hosted plan to keep ingesting. Studio is £78.99/month in the verified GBP catalog:

- **5M events/month** — enough for a SaaS with thousands of active users
- **Unlimited streams** — separate event streams per feature or tenant
- **90-day retention** — vs 14 days on the entry Indie plan
- **x402 agent endpoints** — if you're building AI agent features
- **Hosted MCP (read + write)** — connect Claude Desktop for debugging
...
- **Tier**: Studio (£78.99/month), Scale (£298.99/month), or Enterprise (custom), depending on quotas and retention
...
|---|---|---|
| Setup | Managed account and API key | You deploy and configure it |
| Maintenance | Provider manages the service | You manage upgrades, backups, scaling |
| Data location | Fly.io iad (US East) | Your infrastructure |
| Cost | £18.99-£298.99/month (14-day trial first) | Your compute costs |
...
```
Day 1:    curl → onboard → ingest → query (trial, 5 minutes)
Week 2:   Add SDK, schema validation, separate dev/prod tenants
Month 2:  Pick a paid plan, add monitoring, scope API keys
Month 6+: Scale/Enterprise tier, or self-host if needed
```
