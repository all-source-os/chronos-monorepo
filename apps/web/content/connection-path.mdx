...
**Multi-tenant SaaS.** Each tenant has its own quota (e.g., 500K events/month on Indie, 50M on Scale). The gateway tracks usage per tenant and enforces limits. Implementing that yourself on top of direct Core means reimplementing usage projections, quota checks, and 429 responses — the gateway already does it correctly and consistently with billing.
...
1. **Let the gateway own plan definitions.** The trial and the Indie / Studio / Scale / Enterprise tiers are configured centrally. Don't duplicate them in your app.
