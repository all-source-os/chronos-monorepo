...
prime wins on durability, queryability, history, portability, and team — not by strawman. Markdown genuinely wins on simplicity and a single zero-infra file; prime's auto-compressed export now works too, generated live on demand (see honest limits below for the one residual TODO).
...
- **What works today, proven against the real binary (`allsource-prime 0.21.6`):** recording facets (`prime_add_node` + `prime_embed`), **`prime_recall`** (the relevant-slice recall — the hero feature, demo top hit 0.757), the auto-generated *compressed* voice index (`prime_index` returns the live, populated index — 12 nodes, 5 domains, 77 tokens — so the one-file export and `--auto-inject` work too), `prime_stats`, and `prime_history`. The voice flow runs end to end on these. The earlier 0-node index bug is fixed.
- **The one residual TODO:** the *vector* sub-arm of `prime_context`'s L2 tier still returns an empty list — a documented `// TODO: vector search integration` in the recall engine. `prime_context` returns the populated index correctly; only its vector sub-field is unwired. For the vector-recall path, use `prime_recall`, which works and is what the voice flow already leans on.

We'd rather state the one TODO plainly than dress it up. Recall is the hero, the compressed export is live, and the vector sub-arm of `prime_context` is the single thing still pending.
