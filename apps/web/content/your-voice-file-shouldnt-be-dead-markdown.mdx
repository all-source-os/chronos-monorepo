...
Here's the working hero feature, and it's the whole point. We seeded a real voice file — 12 facets — into the live `allsource-prime` binary (v0.21.4) and asked it to recall for this writing task:
...
| Auto-compressed one-file export | ✅ that's all it is | 🛣️ **roadmap** (next Core release) |

prime wins on durability, queryability, history, portability, and team — not by strawman. Markdown genuinely wins on simplicity and one-file portability today, and the auto-compressed *export* is the one place prime is still catching up (see honest limits below).
...
- **What works today, proven against the real binary:** recording facets (`prime_add_node` + `prime_embed`), **`prime_recall`** (the relevant-slice recall — the hero feature, demo top hit 0.757), `prime_stats`, and `prime_history`. The voice flow runs end to end on these.
- **What's on the roadmap (next Core release), not shipped today:** the auto-generated *compressed* voice-index view, the one-file `/voice export` of that compressed view, and `--auto-inject`-ing the index into every conversation. These read the recall engine's domain-index projection, which in `allsource-prime 0.21.4` reports `0 nodes` for live-recorded facets — a documented Core recall-engine gap, tracked separately. Until it lands, `/voice export` still works by enumerating your facets (just not the token-compressed view), and the live `prime_recall` path — which is the better path anyway — is unaffected.

We'd rather lead with the working magic moment than promise an export that isn't wired. Recall is the hero. The compressed export is coming.
