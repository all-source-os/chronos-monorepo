...
**It speaks MCP.** Agents don't need a bespoke SDK to use it — the first-party [Model Context Protocol](https://modelcontextprotocol.io) connector exposes 55 tenant tools by default, 45 in read-only mode, and 73 with fleet and administrative controls. Claude Desktop or another MCP client can ingest events, query history, reconstruct state, and run analysis through the authenticated gateway.
...
- **Run the benchmark:** `cargo run --release -p allsource-performance` — [step-by-step walkthrough](/blog/reproduce-the-469k-events-benchmark)
