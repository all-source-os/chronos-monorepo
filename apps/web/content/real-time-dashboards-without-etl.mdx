...
The projection updates as accepted events are processed rather than on a batch schedule. The published Core reference benchmark measured 11.9μs p99 indexed reads; browser, network, projection, and API work add end-to-end latency.
...
const socket = new Phoenix.Socket("wss://api.all-source.xyz/socket");
socket.connect();
...
| Dashboard query layer | dbt + query engine | Direct projection lookup; 11.9μs p99 in the Core reference benchmark |
