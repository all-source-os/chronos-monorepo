...
| **MCP server** | Yes — 55 default tenant tools; 73 with fleet/admin controls | Yes — 30 SQL tools |
...
- **The point-read row mixes two different metrics.** stoolap's sub-microsecond figure is an *average* of a prepared SQL point-select; AllSource's 11.9µs is a *p99* indexed-read figure that we did **not** re-measure with the same harness. Do **not** read this as stoolap winning 20× — the metrics (avg vs p99), datasets, and code paths differ. The honest statement: *both serve in-memory point reads in the microsecond range on the cited hardware.*
